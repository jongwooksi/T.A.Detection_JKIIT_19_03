...
class GroundTruthLoader(object):
    ...
    NAME_MAT_MAPPING = {
        ACCIDENT: os.path.join(DATA_DIR, '/home/jwsi/ext_hdd/pgan/dataset/accident/accident.csv'),
    ...
    NAME_FRAMES_MAPPING = {
        ACCIDENT: os.path.join(DATA_DIR, '/home/jwsi/ext_hdd/pgan/dataset/accident/testing'),
    ...
    def __call__(self, dataset):
        ...
        elif dataset == GroundTruthLoader.ACCIDENT:
            gt = self.__load_accident_gt(dataset)          
        else:
            gt = self.__load_ucsd_avenue_subway_gt(dataset)
        return gt

    def __load_accident_gt(self, dataset):
        print("accident_gt")
        import csv
        

        f = open('/home/jwsi/ext_hdd/pgan/dataset/accident/accident.csv', 'r', encoding='utf-8')
        rdr = csv.reader(f)
        idx = 0
        abnormal_events = []
        ...
        for line in rdr:
            ...
            #lines = [line[0].split(';')]
            #print(lines)
            #lines = [list(map(int,lines[0][0].split(','))),list(map(int,lines[0][1].split(',')))]
            #lines =[map(int,line[0]),map(int,line[1])]
            lines = [line[0], line[1]]
            #print(lines)
            abnormal_events.append(np.array(lines,dtype=np.uint16))
            
        f.close()

        #abnormal_event = np.array([[[78,392,503,868,932],[120,422,666,910,1101]]],dtype=np.uint16)
       # print(abnormal_events)
        #abnormal_event = np.append(abnormal_event, np.array([[1,2,3]],[[3,4,5]]),dtype=np.uint16)
        dataset_video_folder = GroundTruthLoader.NAME_FRAMES_MAPPING[dataset]
        video_list = os.listdir(dataset_video_folder)
        video_list.sort()
        num_video = len(abnormal_events)
    ...
    def __load_ucsd_avenue_subway_gt(self, dataset):
        ...
        for i in range(num_video):
            ...
            for j in range(num_abnormal):
                ...
                sub_video_gt[start: end] = 1 # abnomarly section is 1 
...
def precision_recall_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        thrersh = 0.2
        scores[scores < thrersh] = 0
        scores[scores >= thrersh] = 1 
        
    
        print("precision_score: {}".format(metrics.precision_score(labels, scores)))
        print("recall: {}".format(metrics.recall_score(labels, scores)))
        print("f1 score: {}".format(metrics.f1_score(labels, scores)))
...
def compute_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        # video normalization high : normal, low : abnormal
        for i in range(num_videos):
            distance = psnr_records[i]
            ...
            length = int(len(distance)/2)

            #print(np.linspace(0.2, 0,length )).tolist())
            
            #distance[:length] -= np.linspace(0.6, 0, length).tolist()
        ...
        plt.plot(fpr, tpr, linewidth=2, label="ROC curve {:.3f}".format(auc))
        plt.plot([0, 1], [0, 1], 'k--')
        plt.axis([0, 1, 0, 1])
        plt.xlabel('FPR')
        plt.ylabel('TPR')
        ...
        print('mean normal scores = {}, mean abnormal scores = {}, '
            'delta = {}'.format(mean_normal_scores, mean_abnormal_scores, mean_abnormal_scores - mean_normal_scores))
        ...
        plt.plot(thresholds, f1_scores[:-1], "g-", label="F1_scores {:.4f}".format( np.max(f1_scores)))
        ...
        scores[scores >= thresh] = 1
        scores[scores < thresh] = 0

        cfm = metrics.confusion_matrix(labels, scores)
       
        print("confusion matrix: \n [{}, {}] \n [{}, {}]".format(cfm[1][1], cfm[1][0], cfm[0][1], cfm[0][0]))
        print("accuracy: {}".format(metrics.accuracy_score(labels, scores)))
        print("precision: {}".format(metrics.precision_score(labels, scores)))
        print("recall: {}".format(metrics.recall_score(labels, scores)))
        print("f1 score: {}".format(metrics.f1_score(labels, scores)))
        ...
        #plt.plot(np.arange(0.0, 3030, 1), labels, scores,'r-', linewidth=2)
       
        #plt.plot([0, len(labels)], [0, 1])
        #plt.axis([0, len(labels), 0, 1])
        #plt.xlabel('labels')
        #plt.ylabel('scores')
        #plt.legend(loc="lower right")
        #plt.show()
