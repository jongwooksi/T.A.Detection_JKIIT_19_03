...
from models import generator, discriminator, monodepth, initialize_monodepth, flownet, initialize_flownet
from loss_functions import intensity_loss, gradient_loss
from utils import DataLoader, load, save, psnr_error
from constant import const
from bgremover.seg import DeepLabModel,run_visualization
...
height, width = 128, 128 #256 256
flow_height, flow_width = const.FLOW_HEIGHT, const.FLOW_WIDTH
mono_height, mono_width = const.MONO_HEIGHT, const.MONO_WIDTH
l_num = const.L_NUM
...
if lam_flow != 0:
    ...
    #train_input_back = run_visualization(train_inputs, MODEL)
    ...
    print(tf.abs(train_gt_flow-train_pred_flow))
    flow_loss = tf.reduce_mean(tf.abs(train_gt_flow - train_pred_flow))
    #flow_loss = tf.divide(flow_loss, 10)
    print(flow_loss)
else:
...
if lam_mono != 0:
    ...
    train_gt_mono= monodepth(input_image=train_gt, height=mono_height, width=mono_width)
    train_pred_mono= monodepth(input_image=train_outputs,height=mono_height, width=mono_width)
    
    #ssim_loss_gt = tf.reduce_mean(train_gt_mono)
    #ssim_loss_pred = tf.reduce_mean(train_pred_mono)

    #mono_loss = tf.add(ssim_loss_gt, ssim_loss_pred)
    #mono_loss = tf.reduce_mean(tf.square(train_gt_mono - train_pred_mono))

    #mono_loss = -tf.reduce_mean(train_gt_mono*tf.log(train_pred_mono) + (1 - train_gt_mono)*tf.log(1 - train_pred_mono))

    '''
    mono_loss1 = tf.reduce_mean(tf.reduce_sum(tf.abs(train_gt_mono[0] - train_pred_mono[0]),axis=-1))
    mono_loss2 = tf.reduce_mean(tf.reduce_sum(tf.abs(train_gt_mono[1] - train_pred_mono[1]),axis=-1))
    mono_loss3 = tf.reduce_mean(tf.reduce_sum(tf.abs(train_gt_mono[2] - train_pred_mono[2]),axis=-1))
    mono_loss4 = tf.reduce_mean(tf.reduce_sum(tf.abs(train_gt_mono[3] - train_pred_mono[3]),axis=-1))
    '''

    mono_loss1 = tf.reduce_mean(tf.abs(train_gt_mono[0] - train_pred_mono[0]))
    mono_loss2 = tf.reduce_mean(tf.abs(train_gt_mono[1] - train_pred_mono[1]))
    mono_loss3 = tf.reduce_mean(tf.abs(train_gt_mono[2] - train_pred_mono[2]))
    mono_loss4 = tf.reduce_mean(tf.abs(train_gt_mono[3] - train_pred_mono[3]))

    temp1 = tf.add(mono_loss1, mono_loss2)
    temp2 = tf.add(mono_loss3, mono_loss4)
    mono_loss = tf.add(temp1, temp2)
    #mono_loss = tf.add_n(mono_loss1,mono_loss2,mono_loss3,mono_loss4)

    #mono_loss = tf.constant(0.0, dtype=tf.float32)
    #print(mono_loss)
else:
    mono_loss = tf.constant(0.0, dtype=tf.float32)
...
with tf.name_scope('training'):
    g_loss = tf.add_n([lp_loss * lam_lp, gdl_loss * lam_gdl, adv_loss * lam_adv, flow_loss * lam_flow ,mono_loss * lam_mono], name='g_loss')
...
tf.summary.scalar(tensor=mono_loss, name='mono_loss')
tf.summary.scalar(tensor=flow_loss, name='flow_loss')
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    if lam_mono != 0:
        ...
        initialize_monodepth(sess, const.MONODEPTH_CHECKPOINT)
    ...
    while _step < iterations:
        try:
            ...
            _, _g_lr, _step, _lp_loss, _gdl_loss, _adv_loss, _g_loss, _flow_loss, _mono_loss, _train_psnr, _summaries = sess.run(
                [g_train_op, g_lrate, g_step, lp_loss, gdl_loss, adv_loss, g_loss, flow_loss, mono_loss,train_psnr_error, summary_op])
            ...
            if _step % 10 == 0:
                ...
                print('                 monodepth   Loss : ({:.4f} * {:.4f} = {:.4f})'.format(_mono_loss, lam_mono, _mono_loss * lam_mono))
