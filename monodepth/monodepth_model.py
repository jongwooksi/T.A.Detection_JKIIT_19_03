...
class MonodepthModel(object):
    ...
    def build_resnet50(self):
        ...
        #with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('encoder'):
                conv1 = conv(self.model_input, 32, 7, 2) # H/2  -   64D
        ...
        #return self.disp1
        #mono = tf.image.resize_bilinear(self.disp1, tf.stack([256,512]), align_corners=True)
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.elu):
            with tf.variable_scope('model', reuse=self.reuse_variables):
                ...
                if self.params.encoder == 'vgg':
                    ...
                    self.build_resnet50()
                    #return  tf.concat([tf.expand_dims(self.disp1[:,:,:,0],3), tf.expand_dims(self.disp1[:,:,:,1],3)],axis=0)
                else:
                    return None

    def build_outputs(self,input_image):
        # STORE DISPARITIES
        self.model_input = input_image
        ...
        if self.mode == 'test':
            ...
            #return self.disp_left_est[0], self.SSIM( self.left_est[0],  self.left_pyramid[0])

            return  self.disp_left_est
            #return tf.concat([self.disp_left_est[0], self.disp_left_est[0]], axis=1)
